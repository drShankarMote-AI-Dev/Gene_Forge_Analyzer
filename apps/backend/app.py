import os
# Silences some Werkzeug debugger noise
...
if os.environ.get('FLASK_ENV') == 'development' or os.environ.get('NODE_ENV') == 'development' or os.environ.get('NODE_ENV') is None:
    app.debug = True
...
allowed_origins = [url.strip() for url in os.environ.get('ALLOWED_ORIGINS', frontend_url).split(',') if url.strip()]
...
# Environment detection for security hardening
IS_DEV = app.debug or os.environ.get('FLASK_ENV') == 'development' or os.environ.get('NODE_ENV') == 'development'

if IS_DEV:
    import re
    # Permissive CORS for development/local networking
...
DIST_DIR = os.path.join(ROOT_DIR, 'apps', 'frontend', 'dist')
...
def serve_static(path):
    from flask import send_from_directory
    return send_from_directory(os.path.join(DIST_DIR, 'static'), path) if os.path.exists(os.path.join(DIST_DIR, 'static')) else (jsonify({"msg": "Static dir not found"}), 404)
...
def log_request_info():
    # Always log the Origin for CORS debugging in production
...
def admin_login():
    try:
        ...
        if not user:
            app.logger.warning(f"AUTH_FAILURE: User not found for email: {email}")
            check_password_hash('dummy', 'dummy') # Prevent timing attacks
            return jsonify({"msg": "Invalid admin credentials"}), 401

        if user.role != 'admin':
            app.logger.warning(f"AUTH_FAILURE: User {email} has insufficient role: {user.role}")
            return jsonify({"msg": "Invalid admin credentials"}), 401

        if not user.password_hash:
            app.logger.warning(f"AUTH_FAILURE: User {email} has no password set in database")
            return jsonify({"msg": "Invalid admin credentials"}), 401
        ...
        import traceback
        app.logger.error(f"ERROR in admin_login: {e}")
        traceback.print_exc()
        raise e

@app.route('/auth/admin/reset-password-request', methods=['POST'])
def admin_reset_request():
    ...
    try:
        ...
        app.logger.error(f"FAILED TO SEND ADMIN RESET: {e}")
        # Still show code in console for dev
        app.logger.info(f"DEV ADMIN RESET CODE FOR {email}: {reset_code}")
        
    return jsonify({"msg": "If this is a valid admin email, a reset link has been sent."}), 200

@app.route('/auth/admin/reset-password-confirm', methods=['POST'])
...
def send_otp():
    ...
    try:
        ...
        try:
            ...
            """
            mail.send(msg)
            app.logger.debug(f"Email sent successfully to {email}")
        except Exception as e:
            ...
            if not is_configured:
                app.logger.warning(f"Email NOT SENT to {email} because MAIL_USERNAME/PASSWORD are not set in .env")
            else:
        ...
        if is_dev:
            ...
            app.logger.info(f"DEV PASSCODE FOR {email}: {code}")
...
@app.route('/auth/google/verify-token', methods=['POST'])
def google_verify_token():
    ...
    try:
        ...
        app.logger.error(f"Unexpected OAuth error: {str(e)}")
...
@app.route('/genomic-data', methods=['POST'])
@jwt_required()
...
@app.route('/genomic-data', methods=['GET'])
@jwt_required()
...
@app.route('/genomic-data/<int:data_id>', methods=['GET'])
@jwt_required()
...
@app.route('/projects/<int:project_id>/analysis', methods=['POST'])
@jwt_required()
...
@app.route('/projects/<int:project_id>/analysis', methods=['GET'])
@jwt_required()
...
def ai_explain_stream():
    ...
    def generate():
        ...
        try:
            ...
            app.logger.error(f"CRITICAL STREAM ERROR: {e}")
            with app.app_context():
                u = User.query.get(user.id)
                usage = AIUsage(
                    user_id=u.id,
...
if __name__ == '__main__':
    ...
    app.debug = debug # Ensures the detailed error handlers and cookie logic work correctly
    ...
    with app.app_context():
        ...
        if not admin:
            app.logger.info(f"Seeding default admin account: {admin_email}")
            salt = os.urandom(16)
            admin = User(
                email=admin_email,
                role='admin',
    ...
    # We apply these here because 'app.debug' might not be set at module level during import
    if debug:
        app.logger.info("DEBUG: Applying Development Overrides (CORS & Cookies)")
        
        # 1. Relax Cookies for HTTP (Local Network)
        app.config['JWT_COOKIE_SECURE'] = False
        app.config['JWT_COOKIE_SAMESITE'] = 'Lax'
        
        # 2. Allow All Origins for Development
        # This fixes issues where accessing via IP (192.168.x.x) fails CORS
        from flask_cors import CORS
        CORS(app, supports_credentials=True, resources={r"/*": {"origins": "*"}})
        socketio = SocketIO(app, cors_allowed_origins="*", manage_session=False)
        app.logger.info("DEBUG: CORS and SocketIO set to allow ALL origins (*)")

    try:
        app.logger.info(f"Initializing Analysis Engine on port {port}...")
        app.logger.info(f"CORS allowed origins: {allowed_origins}")
        app.logger.info(f"Static assets directory: {DIST_DIR}")
        # On Windows, we need allow_unsafe_werkzeug=True for SocketIO with debug
        socketio.run(app, host='0.0.0.0', port=port, debug=debug, allow_unsafe_werkzeug=True)
    except Exception as e:
        if "10048" in str(e):
            app.logger.warning(f"PORT {port} CONFLICT: Attempting recovery on backup port...")
            socketio.run(app, host='0.0.0.0', port=port + 1, debug=debug, allow_unsafe_werkzeug=True)
        else:
            raise e

# Catch-all route for SPA (React Router) - must be at the very bottom

@app.route('/', defaults={'path': ''})
@app.route('/<path:path>')
def catch_all(path):
    # Skip API routes and static assets that might have leaked here
    api_prefixes = ['auth/', 'admin/', 'genomic-data/', 'projects/', 'analysis/', 'ai/', 'health', 'assets/', 'static/']
    if any(path.startswith(prefix) for prefix in api_prefixes) or '.' in path:
    ...
    if os.path.exists(os.path.join(DIST_DIR, 'index.html')):
        ...
        return jsonify({
            "msg": "Gene Forge API is Online",
            "frontend": "Not Built (Run 'npm run build' in apps/frontend)",
            "status": "ready"
        }), 200
